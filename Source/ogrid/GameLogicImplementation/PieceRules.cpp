...
namespace OGRID
{
    ...
    bool NormalCheckersMoveRule::IsValidMove(Grid *grid, int fromX, int fromY, int toX, int toY) const
    {
        ...
        // -1 = up, 1 = down
        int moveDirection = grid->GetPieceAt(fromY, fromX)->GetOwner()->GetSide() == 0 ? -1 : 1;
        // If white -> go up, if black -> go down
    ...
    {
        // Can't jump onto a cell that's occupied
        if (grid->GetPieceAt(toY, toX) != nullptr)
        {
        ...
        if (toY == fromY + moveDirectionY && (toX == fromX + moveDirectionY || toX == fromX - moveDirectionY))
        {
            ...
            if (toX == fromX + moveDirectionY)
                enemyPieceX = fromX + enemyMoveDirectionY;
            else if (toX == fromX - moveDirectionY)
                enemyPieceX = fromX - enemyMoveDirectionY;
        }
        else
        {
        ...
        // If the jump is possible on Y axis
        if (toY + moveDirectionY < grid->GetRows() && toY + moveDirectionY >= 0)
        {
            // And on the X axis to the right
            if (toX + 2 < grid->GetCols())
            {
                // If jump space is empty
                if (grid->GetPieceAt(toY + moveDirectionY, toX + 2) == nullptr)
                {
                    // And there's something to jump over
    ...
    bool JumpSuperCheckersAttackRule::IsValidAttack(Grid *grid, int fromX, int fromY, int toX, int toY, bool &canContinue) const
    {
        ...
        while (!canContinue)
        {
            ...
            for (deltaY = 1, deltaX = 1; toY + deltaY < grid->GetRows() && toX + deltaX < grid->GetCols(); deltaY += 1, deltaX += 1)
            {
                ...
                if (grid->GetPieceAt(toY + deltaY, toX + deltaX) == nullptr)
                {
                    // If jump space is empty
                    // And there's something to jump over,
                    // not including yourself (pre-move)
                    if (grid->GetPieceAt(toY + deltaY - 1, toX + deltaX - 1) != nullptr &&
                        (toY + deltaY - 1 != fromY && toX + deltaX - 1 != fromX) &&
                        (toY + deltaY - 1 != firstFoundPieceY && toX + deltaX - 1 != firstFoundPieceX))
                    {
                        // Or the previously jumped over piece
                        if (grid->GetPieceAt(toY + deltaY - 1, toX + deltaX - 1)->GetOwner()->GetSide() != side)
                        {
                            // And it's an enemy
                            canContinue = true;
                            break;
            ...
            for (deltaY = 1, deltaX = -1; toY + deltaY < grid->GetRows() && toX + deltaX >= 0; deltaY += 1, deltaX -= 1)
            {
            ...
            for (deltaY = -1, deltaX = -1; toY + deltaY >= 0 && toX + deltaX >= 0; deltaY -= 1, deltaX -= 1)
            {
            ...
            for (deltaY = -1, deltaX = 1; toY + deltaY >= 0 && toX + deltaX < grid->GetCols(); deltaY -= 1, deltaX += 1)
            {
